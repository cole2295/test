...
loadnav = function () {
    ...
    tablist = $("#tab_list_add").Tab({
        items: [
            { id: 'desktop', title: '欢迎首页', closed: false, icon: 'fa fa fa-desktop', url: contentPath + '/Admin/Home/Desktop' },
        ],
        tabScroll: false,
        ...
        addEvent: function (item) {
            if (item.closed && item.isNoLog != true) {
                $.ajax({
                    url: contentPath + "/Admin/Home/VisitModule",
    ...
    $("#UserSetting").click(function () {
        tablist.newTab({ id: "UserSetting", title: "个人中心", closed: true, icon: "fa fa fa-user", url: contentPath + "/Admin/PersonCenter/Index" });
    });
    //动态加载导航菜单
    var _html = "";
    var index = 0;
            ...
            window.setTimeout(function () {
                $.ajax({
                    ...
                    success: function (data) {
                        window.location.href = contentPath + "/Admin/Login/Index";
                    }
                });
            }, 500);
        }
